...
namespace CoreApiFundamentals.Data
{
    public class CampRepository : ICampRepository
    {
        ...
        public async Task<Camp[]> GetAllCampAsync(bool includeTalks = false)
        {
            _logger.LogInformation($"Getting all Camps");
        ...
        public async Task<Camp> GetCampAsync(string Moniker, bool includeTalks = false)
        {
            ...
            if (includeTalks)
            {
                query = query.Include(m => m.Talks).ThenInclude(c => c.Speaker);
            }

            //query it
            query = query.Where(m => m.Moniker == Moniker);
        ...
        public async Task<Talk[]> GetTalksByMonikerAsync(string moniker, bool includeSpeakers = false)
        {
            ...
            if (includeSpeakers)
            {
                query = query.Include(m => m.Speaker);
            }

            //add query
            query = query.Where(m => m.Camp.Moniker == moniker)
        ...
        public async Task<Talk> GetTalkByMonikerAsync(string moniker, int talkId, bool includeSpeakers = false)
        {
            ...
            //add query
            query = query.Where(m => m.TalkId == talkId && m.Camp.Moniker == moniker);
